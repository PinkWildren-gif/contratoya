...
export function EditDocument() {
  ...
  if (!document) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 text-center">
        <h1 className="text-2xl font-serif font-bold text-navy-800 mb-4">Documento no encontrado</h1>
        <Link to="/documents"><Button variant="outline">Volver a mis documentos</Button></Link>
      </div>
    )
  }
  ...
  if (!config) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 text-center">
        <h1 className="text-2xl font-serif font-bold text-navy-800 mb-4">Tipo de documento no disponible</h1>
        <Link to="/documents"><Button variant="outline">Volver</Button></Link>
      </div>
    )
  }
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-serif font-bold text-navy-800 mb-2">Documento actualizado</h1>
        </div>
        ...
        {pdfDataUrl && (
          <div className="card overflow-hidden mb-8">
            <iframe src={pdfDataUrl} className="w-full h-[600px] border-0" title="Vista previa" />
          </div>
        )}
        <div className="flex gap-4 justify-center">
          <Button variant="ghost" onClick={() => navigate('/documents')}>Mis documentos</Button>
          <Button variant="outline" onClick={() => { setGenerated(false); setPdfDataUrl(null) }}>Editar de nuevo</Button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-serif font-bold text-navy-800">Editar: {document.title}</h1>
      </div>
      <Questionnaire
        config={config}
